...
class Identification_manager:
    ...
    def __init__(self):
        ...
        '''
        rospy.Subscriber('/asv1/fix_ublox', NavSatFix, self.gps1_ublox_callback)
        rospy.Subscriber('/asv1/vehicle/state', Vector, self.gps1_position_callback)
        rospy.Subscriber('/asv1/fix_velocity', Vector3Stamped, self.gps1_velocity_callback)
        rospy.Subscriber('/asv1/imu', Imu, self.imu1_callback)
        rospy.Subscriber('/asv1/imu_vect_gyro',Vector3,self.imu1_gyro_callback)
        rospy.Subscriber('/asv1/low_level/vehicle/tau', String, self.real1_thrust_callback)


        rospy.Subscriber('/asv1/controller/state', Vector, self.state1_controller_callback)
        rospy.Subscriber('/asv1/vehicle/reference', Vector, self.state1_g_callback)
        '''

        rospy.Subscriber('/asv2/fix_ublox', NavSatFix, self.gps_ublox_callback)
        rospy.Subscriber('/asv2/vehicle/state', Vector, self.gps_position_callback)
        rospy.Subscriber('/asv2/fix_velocity', Vector3Stamped, self.gps_velocity_callback)
        rospy.Subscriber('/asv2/imu', Imu, self.imu_callback)
        rospy.Subscriber('/asv2/imu_vect_gyro',Vector3,self.imu_gyro_callback)
        rospy.Subscriber('/asv2/low_level/vehicle/tau', String, self.real_thrust_callback)


        rospy.Subscriber('/asv2/controller/state', Vector, self.state_controller_callback)
        rospy.Subscriber('/asv2/vehicle/reference', Vector, self.state_g_callback)
        rospy.Subscriber('/asv2/vehicle_soc_battery/data', BatteryStatus, self.battery_callback)



        '''
        self.tau1 = np.array((0,0,0))
        self.tau1 = np.reshape(self.tau1,(3,1))
        self.position_gps1 = np.zeros((3,1))
        self.position_gps_ublox1 = np.zeros((3,1))
        self.controller_state1 = np.zeros((3,1))
        self.admissible_reference1 = np.zeros((3,1))
        self.orietation_imu1 = np.zeros((4,1))
        self.orientation_yaw1 = 0.0
        self.acc_imu1 = np.zeros((3,1))
        self.gyro1 = np.zeros((3,1))
        self.normalized_speed1 = np.zeros((3,1))
        '''
        ...
        self.batterystat = BatteryStatus() 
    ...
    def collect_data(self, time_duration):
        ...
        battery_info = np.zeros((3, time_duration)) 
        ...
        while not rospy.is_shutdown() and counter < time_duration:
            ...
            battery_info[0, counter] = float(self.batterystat.voltage)
            battery_info[1, counter] = float(self.batterystat.current)
            battery_info[2, counter] = float(self.batterystat.level)

            rate.sleep()
            counter = counter + 1

        return (position, position_ublox, orientation, orientation_yaw, acceleration, gyro, v_speed, tau_applied, controller_state, admissible_reference, battery_info)
    ...
    def loop(self):
        ...
        ############ COLLECT SPEED DATA 
            ...
            (position, position_ublox, orientation, orientation_yaw, acceleration, gyro, v_speed, tau, controller_state, admissible_reference, battery_info) = self.collect_data(samples)
            ...
            "tau":tau, "t":t, "controller_state":controller_state, "admissible_reference": admissible_reference, "battery_info": battery_info}
            io.savemat("/home/udoo/ares_ros_ws/CollisionAvoidancebase5_asv2.mat", data)
            return 0
